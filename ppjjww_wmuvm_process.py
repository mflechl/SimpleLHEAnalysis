...
def name_particles(particles):
    ...
    for particle in particles:
        ...
        if particlestatus in {1,2}: #products and temporary products
            if particleid in z: 
                particleid = "z"
                particle2count = z2count
            elif particleid in jet:
                particleid = "j"
                particle2count = jet2count
            elif particleid in b:
                particleid = "b"
                particle2count = b2count
            elif particleid in l:
                particleid = "l"
                particle2count = l2count
        elif particlestatus in {-1} and (particleid in jet or particleid in b): #substracts
            particleid = "q"
...
def calculate_dependent_variables(particles):
    ...
    if not particles.has_key('j_1'):
        return {}

    if not particles.has_key('j_2'):
        return {}

    j1 = particles["j_1"]

    if particles.has_key('j_2'): 
        j2 = particles["j_2"]
        nj=2
    else: 
        j2 = particles["j_1"]
...
#    update_variables_by_single(variables, particles, eta_ma, "etama")
    ...
    if nj==2:
        ...
        update_variables_by_unordered_pair(variables, particles, delta_eta, "delta_eta_")
